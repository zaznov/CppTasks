//============================================================================
// Name        : red_week4_AirportCounter.cpp
...
public:
  ...
  AirportCounter(TIterator begin, TIterator end){	//принимает диапазон вылетов нашей авиакомпании и сохраняет в список вылетов нашей авиакомпании
	  flights.fill(0);

	  for(TIterator this_airport = begin; this_airport != end; this_airport++){
		  Insert(*this_airport);
	  }
  }
  size_t Get(TAirport airport) const{
	  return flights.at(static_cast<size_t>(airport));
  }

  void Insert(TAirport airport){
	  flights.at(static_cast<size_t>(airport))++;
  }
  ...
  void EraseAll(TAirport airport){
	  flights.at(static_cast<size_t>(airport)) = static_cast<size_t>(0);
  }
  ...
  Items GetItems() const{
	  ...
	  {
		  items[i] = {static_cast<TAirport>(i), flights[i]};
	  }
	  return items;
  }
  ~AirportCounter(){
...
private:
  ...
  array<size_t, static_cast<size_t>(TAirport::Last_)> flights;
};

void TestMoscow() {
  enum class MoscowAirport {		//это тип
    VKO,
    SVO,
    DME,
    ZIA,
  ...
  const vector<MoscowAirport> airports = {		//а это список вылетов нашей авиакомпании из тех аэропортов где работает наша авиакомпания
      MoscowAirport::SVO,
      MoscowAirport::VKO,
      MoscowAirport::ZIA,
      MoscowAirport::SVO,
...
#define ASSERT_EQUAL_ITEM(idx, expected_enum, expected_count) \
  ...
  /*cout <<  static_cast<size_t>(items[0].first) << endl;
  cout <<  static_cast<size_t>(items[1].first) << endl;
  cout <<  static_cast<size_t>(items[2].first) << endl;
  cout <<  static_cast<size_t>(items[3].first) << endl;*/
